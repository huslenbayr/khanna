...
import type { CommunityMember, CommunityRole, CommunityStatus } from '../types/community'

const STORAGE_KEY = 'khannaway-community-home-v1'
const HOME_NAME_KEY = 'khannaway-community-home-name-v1'

type CommunityHomePanelProps = {
...
const CommunityHomePanel = ({
  members,
  selectedMemberId,
  onMembersChange,
  onSelectMember,
}: CommunityHomePanelProps) => {
  ...
  useEffect(() => {
    try {
      ...
      // Keep seeded demo members if local storage is unavailable.
    }
  ...
  return (
    <div className="glass-panel overlay-panel animate-fade-in community-home-panel" style={{
      top: '5.25rem',
      right: '2rem',
      width: '430px',
      maxHeight: '84vh',
      ...
      <label style={labelStyle}>
        Гэрийн нэр
        <input value={homeName} onChange={(event) => setHomeName(event.target.value)} placeholder="Жишээ: 11-р ангийн найзууд" style={fieldStyle} />
      </label>
      ...
      {showForm && (
        <form onSubmit={addMember} style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.65rem' }}>
            <label style={labelStyle}>
              Нэр
              <input value={form.name} onChange={(event) => updateForm('name', event.target.value)} placeholder="Найзын нэр" style={fieldStyle} />
            </label>
            <label style={labelStyle}>
              Хамаарал
          ...
          <label style={labelStyle}>
            Байршил
            <input value={form.location} onChange={(event) => updateForm('location', event.target.value)} placeholder="Жишээ: Сансар, Blue Sky, гэрийн ойролцоо..." style={fieldStyle} />
          </label>
...
export default CommunityHomePanel
