...
const emptyForm: CommunityForm = { name: '', role: 'Найз', phone: '', location: '', status: 'Онлайн', lat: '', lng: '' }
...
const normalizeStatus = (status: string): CommunityStatus => {
  if (status === 'Онлайн' || status === 'Офлайн' || status === 'Хамт байна') return status
  return 'Онлайн'
}

const TeamTracker = ({ members, selectedMemberId, onMembersChange, onSelectMember }: TeamTrackerProps) => {
  ...
  useEffect(() => {
    try {
      ...
      if (savedHomeName) setHomeName(savedHomeName)
      if (Array.isArray(savedMembers) && savedMembers.length > 0) {
        onMembersChange(savedMembers.map(m => ({ ...m, status: normalizeStatus(m.status), updatedAt: m.updatedAt || new Date().toISOString() })))
      }
    } catch { /* ignore */ }
  ...
  const summary = useMemo(() => ({
    online: members.filter(m => m.status === 'Онлайн').length,
    withYou: members.filter(m => m.status === 'Хамт байна').length,
  }), [members])

  const updateForm = <F extends keyof CommunityForm>(field: F, value: CommunityForm[F]) => {
  ...
  const addMember = (event: FormEvent<HTMLFormElement>) => {
    ...
    const member: CommunityMember = {
      ...
      location,
      status: form.status,
      coordinates: { lat: Number(form.lat) || fallback.lat, lng: Number(form.lng) || fallback.lng },
      updatedAt: new Date().toISOString(),
    }
    onMembersChange([member, ...members])
  ...
  const useDeviceLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(pos => {
      setForm(c => ({ ...c, lat: pos.coords.latitude.toFixed(6), lng: pos.coords.longitude.toFixed(6), location: c.location || 'Миний одоогийн байршил' }))
    })
  }

  return (
    <div className="glass-panel overlay-panel animate-fade-in" style={{ top: '5.25rem', right: '2rem', width: '430px', maxHeight: '84vh', overflowY: 'auto', padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem', borderBottom: '1px solid var(--border-glass)', paddingBottom: '0.85rem' }}>
        ...
        <button className="glass-button" onClick={() => setShowForm(v => !v)} style={{ padding: '0.55rem 0.65rem' }}>
          <Plus size={16} /> Нэмэх
        </button>
      </div>
      ...
      <label style={labelStyle}>
        Гэрийн нэр
        <input value={homeName} onChange={e => setHomeName(e.target.value)} placeholder="Гэрийн нэр" style={fieldStyle} />
      </label>

      {showForm && (
        <form onSubmit={addMember} style={{ display: 'grid', gap: '0.75rem', padding: '0.85rem', background: 'rgba(255, 255, 255, 0.05)', border: '1px solid var(--border-glass)', borderRadius: 8 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.65rem' }}>
            <label style={labelStyle}>Нэр <input value={form.name} onChange={e => updateForm('name', e.target.value)} placeholder="Нэр" style={fieldStyle} /></label>
            <label style={labelStyle}>Хамаарал
              <select value={form.role} onChange={e => updateForm('role', e.target.value as CommunityRole)} style={fieldStyle}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.65rem' }}>
            <label style={labelStyle}>Утас <input value={form.phone} onChange={e => updateForm('phone', e.target.value)} placeholder="+976 ..." style={fieldStyle} /></label>
            <label style={labelStyle}>Төлөв
              <select value={form.status} onChange={e => updateForm('status', e.target.value as CommunityStatus)} style={fieldStyle}>
          ...
          <label style={labelStyle}>Байршил <input value={form.location} onChange={e => updateForm('location', e.target.value)} placeholder="Жишээ: Сансар..." style={fieldStyle} /></label>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '0.65rem', alignItems: 'end' }}>
            <label style={labelStyle}>Lat <input value={form.lat} onChange={e => updateForm('lat', e.target.value)} placeholder="47.9186" style={fieldStyle} /></label>
            <label style={labelStyle}>Lng <input value={form.lng} onChange={e => updateForm('lng', e.target.value)} placeholder="106.9177" style={fieldStyle} /></label>
            <button type="button" className="glass-button" onClick={useDeviceLocation} style={{ padding: '0.65rem 0.72rem' }}><Navigation size={15} /></button>
          </div>
        ...
        {members.map(member => (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.85rem' }}>
              <div style={{ width: 42, height: 42, borderRadius: '50%', background: statusColors[member.status], display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#06111a', flex: '0 0 auto', fontWeight: 800 }}>
                {member.name.slice(0, 1).toUpperCase()}
              </div>
              <div style={{ minWidth: 0, flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.5rem' }}>
                  <div style={{ fontSize: '0.95rem', fontWeight: 700 }}>{member.name}</div>
                  <span role="button" tabIndex={0} onClick={e => { e.stopPropagation(); removeMember(member.id) }} style={{ color: 'var(--text-muted)', cursor: 'pointer' }}>
                    <Trash2 size={15} />
                  </span>
                </div>
                <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginTop: '0.18rem' }}>{member.role} • {statusLabels[member.status]}</div>
              </div>
            </div>
