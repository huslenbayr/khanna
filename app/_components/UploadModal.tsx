...
const MB = 1024 * 1024
const VIDEO_COMPRESS_THRESHOLD = 20 * MB // compress videos over 20MB
...
export default function UploadModal({
  ...
  const reset = () => {
    ...
    setLN(''); setLat(defaultLat); setLng(defaultLng)
  ...
  const handlePost = async () => {
    ...
    if (file.type.startsWith('video/') && file.size > VIDEO_COMPRESS_THRESHOLD) {
      try {
        ...
        setProgress(0)
        setStatusMsg('Видео ачааллаж байна...')
        uploadFile = await compressVideo(file, (substage, pct) => {
          if (substage === 'loading') {
            setStatusMsg('FFmpeg ачааллаж байна...')
            setProgress(Math.round(pct * 0.3))
          } else {
            setStatusMsg('Видео боловсруулж байна...')
            setProgress(30 + Math.round(pct * 0.5))
          }
        })
        setProgress(80)
        setStatusMsg('Бичлэг бэлэн боллоо')
      } catch {
        setStage('error')
        setError('Видео боловсруулахад алдаа гарлаа')
        return
      }
    }
    ...
    try {
      ...
      setStatusMsg('Cloudflare руу илгээж байна...')
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="h-[3px] bg-[var(--border-glass)] rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${progress}%`,
                  background: stage === 'compressing' ? '#a78bfa' : 'var(--primary)',
                }}
              />
            </div>
          </div>
